...
class FieldMapper:
    ...
    def ensure_mappings_directory(self):
        ...
        product_dirs = [
            ...
            "application", "cloud", "network", "product", "category"
        ]
...
class DetectionLogicTranslator:
    ...
    def translate_condition(self, field: str, operator: str, values: Union[str, List[str]]) -> str:
...
class MetadataMapper:
    ...
    def map_tags(cls, sigma_tags: List[str]) -> List[str]:
        ...
        for tag in sigma_tags:
            if tag.startswith('attack.'):
                ...
                if tag in cls.MITRE_TACTIC_MAPPINGS:
                    mitre_tactics.append(f'_mitreAttackTactic:{cls.MITRE_TACTIC_MAPPINGS[tag]}')
            ...
            elif tag.lower() in ['detection.emerging_threats', 'detection.threat_hunting', 'sysmon']:
        ...
        if cve_tags:
            ...
            sorted_cves = sorted(cve_tags, key=lambda x: x.split('-')[1] if '-' in x else '0000', reverse=True)
        ...
        if len(mitre_techniques) > MAX_TECHNIQUES:
            overflow_notes.append(f'+{len(mitre_techniques) - MAX_TECHNIQUES}_techniques')
            ...
            if ':' in line:
                ...
                if indent > 0 and current_key:
                    ...
                    if value.startswith('[') and value.endswith(']'):
                        # Inline list
                        result[current_key][key] = [item.strip().strip('"\'') for item in value[1:-1].split(',')]
                    else:
                        result[current_key][key] = value.strip('"\'')
                else:
                    if value.startswith('[') and value.endswith(']'):
                        # Inline list
                        result[key] = [item.strip().strip('"\'') for item in value[1:-1].split(',')]
                    else:
...
class SigmaToSumoConverter:
    ...
    def convert_sigma_rule(self, sigma_yaml: str) -> ConversionResult:
        ...
        try:
            ...
            if HAS_YAML:
                ...
                print("⚠️  PyYAML not found, using simple parser (limited functionality)")
            ...
            if not sigma_rule.get('detection'):
                return ConversionResult(False, errors=['Missing required field: detection'])
            ...
            detection_expr = self.logic_translator.translate_detection(sigma_rule['detection'])
            
            # Add log source conditions
            if 'logsource' in sigma_rule:
                logsource_conditions = self.logsource_mapper.map_logsource(sigma_rule['logsource'])
            ...
            severity_score = self.metadata_mapper.get_severity_score(sigma_rule.get('level', 'medium'))
            ...
            api_json = {
                "fields": {
                    ...
                    "parentJaskId": None,
                    "summaryExpression": description[:200] + "..." if len(description) > 200 else description,
                    "tags": tags,
                    "tuningExpressionIds": [],
    ...
    def convert_file(self, input_file: str, output_dir: str = "output") -> bool:
        ...
        try:
            ...
            category_dir = Path(output_dir) / "sumo_cse_rules" / category.lower().replace(' ', '_')
...
Examples:
  ...
  python3 sigma_sumo_converter.py -d /path/to/sigma/rules -o /path/to/output
    ...
    group.add_argument('-d', '--directory', help='Input directory containing Sigma rule files')
    
    parser.add_argument('-o', '--output', default='output', 
                       help='Output directory (default: output)')
    
    args = parser.parse_args()
    print(f"📋 Arguments: input={args.input}, directory={args.directory}, output={args.output}")
